...
export default function GridImages({ images, cols }: SlideshowProps) {
  ...
  return (
    <div id="scroll" className={`grid gap-4 sm:gap-6 md:gap-8 xl:gap-12 grid-cols-1 md:grid-cols-2 m-4 md:m-0  `}>
      {images.map((image, index) => (
        <a key={index} href={image} data-pswp-width="1875" data-pswp-height="2500" className="w-full">
          <img
            ...
            src={image}
            alt={`Image ${index + 1}`}
            className={`object-cover object-center w-full h-[1100px] rounded-lg ${
